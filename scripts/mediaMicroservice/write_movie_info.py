...
def main(
  ...
  future = asyncio.ensure_future(write_cast_info(server_address, raw_casts))
  loop.run_until_complete(future)

  with open(movie_filename, 'r') as movie_file:
    raw_movies = json.load(movie_file)
    loop = asyncio.get_event_loop()
    future = asyncio.ensure_future(write_movie_info(server_address, raw_movies))
    loop.run_until_complete(future)
...
if __name__ == '__main__':
  ...
  parser.add_argument("--server_address", action="store", dest="server_address",
          type=str, default="http://localhost:30445",
